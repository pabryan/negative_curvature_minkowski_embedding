...
\]
Similarly, the adjugate
\[
\adj_g B = g(\adj\op{B} \cdot, \cdot)
\]
is a \emph{non linear} function taking bilinear forms to bilinear forms. The construction respects compositions in the sense that
\[
...
To prove the theorem, first we need to compute the linearisation of the operator \(\adj \circ \opGrav\) at \(S\) such that \(\opGrav(S)\) is invertible.
