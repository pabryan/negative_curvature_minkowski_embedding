...
\chapter{Geometrisation Of Three Manifolds}
...
By \Cref{thm:intg_embed} we may embed \((\tilde{M}, \tilde{g})\) into Minkowski space as a locally convex, co-compact (by ambient isometry), spacelike hypersurface. By \cite[Theorem 1.1]{MR3344442}, the rescaled Gauss curvature flow deforms \((\tilde{M}, \tilde{g})\) smoothly to the hyperboloid at infinity with constant negative sectional curvature. Thus the flow provides a smooth homotopy from \((\tilde{M}, \tilde{g})\) to \((\tilde{M}, \tilde{g}_{\H})\).
...
\chapter{The Cross Curvature Flow}
\label{sec:xcf}
\section{Definition And Basic Properties Of The Flow}
...
The proof of \Cref{thm:xcf_existence_uniqueness} makes use of the DeTurck XCF,
\begin{equation}
\label{eq:dtxcf}
...
\section{Basic Identities And Evolution Equations}
...
\section{Towards Hyperbolic Convergence}
...
\section{Harnack inequality And Solitons}
...
\section{Monotonicity Of Einstein Volume}
