...
Conversely, suppose \(\Ob\) is Codazzi and let \(\mathcal{O}\) be such that \(\Ob(X, Y) = g(\mathcal{O}(X), Y)\). Then \(\Ob\) being Codazzi is precisely the integrability condition required to locally integrate the over-determined system
\begin{align*}
...
\begin{lemma}
\label{lem:dadjG}
Let \(S : V \to V\) be an isomorphism of vector spaces. Then the linearisation of \(\adj \circ \opGrav\) at \(S\) is given by
\[
