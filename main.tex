...
Let \((N, k)\) be a compact Riemannian three-manifold of strictly negative sectional curvature with the integrability condition that the tensor \(O = \sqrt{\det \Ein} \Ein^{-1}\) is \emph{Codazzi}. Then \(N\) admits a metric of constant, negative sectional curvature.
\end{thm}
Here \(\Ein = \Ric - \tfrac{\Sc}{2}k\) denotes the Einstein tensor. Writing \(\Ein(X, Y) = k(\opEin(X), Y)\) we define \(\det \Ein = \det \opEin\) and \(\Ein^{-1}(X, Y) = k(\opEin^{-1} (X), Y)\) whenever \(\opEin\) is invertible. To say that \(O\) is Codazzi means that the three-tensor \(\nabla O\) is totally symmetric.
\begin{rem}
Note that by \Cref{lem:eins_sectional} below, in three dimensions, \(k\) has strictly negative sectional curvature if and only if \(\Ein\) is strictly positive definite. In that case, we see that \(\opEin\) is a strictly positive-definite endomorphism and in particular invertible. Thus \(\Ein\) and \(O\) are well defined.
\end{rem}
...
Let \((N, k)\) be a compact Riemannian three-manifold of strictly negative sectional curvature. Then the tensor \(O = \sqrt{\det \Ein} \Ein^{-1}\) is Codazzi (integrability condition) if and only if the Riemannian universal cover \((M, g = \pi^{\ast} k)\) embeds isometrically into Minkowski space \(\R^{3,1}\) as a locally convex, co-compact, spacelike hypersurface.
\end{thm}
\begin{proof}
If \((M, g)\) embeds isometrically into \(\R^{3,1}\) then by the Codazzi-Mainardi equation \eqref{eq:codazzi}, the second fundamental form \(A\) is Codazzi. Then \Cref{lem:ein_W} below gives \(A = O\), hence \(O\) is Codazzi.

Conversely, suppose \(O\) is Codazzi and let \(\mathcal{O}\) be such that \(O(X, Y) = g(\mathcal{O}(X), Y)\). Then \(O\) being Codazzi is precisely the integrability condition required to locally integrate the over-determined system
\begin{align*}
...
\begin{align*}
\Rm &= -\Sch \owedge g + \mathrm{W} \\
&= -\frac{1}{n-2} \left(\Ric - \frac{R}{2(n-1)} g\right) \owedge g + \mathrm{W} \\
&= -\frac{1}{n-2} \left(\Ein  -\frac{1}{n-1} \Tr_g \Ein g\right) \owedge g + \mathrm{W}
\end{align*}
where \(\mathrm{W} = \Rm + \Sch \owedge g\) is the Weyl tensor, and the Kulkarni-Nomizu product of symmetric bilinear forms \(A, B\) is defined as
\[
\begin{split}
A \owedge B (X, Y, Z, \mathrm{W}) &= A \owedge B (X \wedge Y, Z \wedge \mathrm{W}) \\
&= A(X, Z) B(Y, \mathrm{W}) + A(Y, \mathrm{W}) B(X, Z) \\
&\quad - A(X, \mathrm{W}) B(Y, Z) - A(Y, Z) B(\mathrm{W}, \mathrm{W}).
\end{split}
\]
...
O := \sqrt{\det \opEin} \Ein^{-1} = A
\]
and hence \(O\) is Codazzi since by the Codazzi-Mainardi equations, \(A\) is Codazzi.
\end{proof}
\begin{rem}
...
\begin{align}
 u&=\log\det\Ein,\quad
O_{ij}=\sqrt{\det\Ein}V_{ij},\\
\mathrm{T}^{kij}&=\Ein^{kl}\nabla_l \Ein^{ij},\quad \mathrm{T}^i=V_{jk}\mathrm{T}^{ijk}=\Ein^{ij}\nabla_ju.
\end{align}
For some tensor $\mathrm{L}^{ijk}$ whose traces with respect to $V_{kl}$ are zero, we have
\begin{align}
    \mathrm{T}^{ijk}-\mathrm{T}^{jik}&=\mathrm{L}^{ijk}-\mathrm{L}^{jik}+\frac{1}{2}\left(\mathrm{T}^i\Ein^{jk}-\mathrm{T}^j\Ein^{ik}\right).
\end{align}
The Devil tensor is defined by
\[\mathrm{D}^{ijk}=\mathrm{L}^{ijk}-\mathrm{L}^{jik}\]
and it satisfies interesting identities such as
\begin{align}
\mathrm{D}^{ijk}=&-\mathrm{D}^{jik},\quad \mathrm{D}^{ijk}+\mathrm{D}^{kij}+\mathrm{D}^{jki}=0,\\
V_{ij}\mathrm{D}^{ijk}=&V_{ik}\mathrm{D}^{ijk}=V_{jk}\mathrm{D}^{ijk}=0,\\
V_{ij}\nabla_k\mathrm{D}^{kij}=&\frac{1}{2}|\mathrm{D}^{ijk}|_V^2.
\end{align}
\begin{lemma}
\label{lem:cubicform_codazzi}
We have the following identity.
\begin{align}
|\mathrm{D}^{ijk}|_V^2=\frac{1}{\det\Ein}|\nabla_iO_{jk}-\nabla_jO_{ik}|_{\Ein}^2.
\end{align}
In particular, \(\mathrm{D}\equiv 0\) if and only if \(O\) is Codazzi.
\end{lemma}
\begin{proof}
Using the definition we calculate
\begin{align}
\mathrm{T}^{kij}=&-\Ein^{kl}\Ein^{im}\Ein^{jn}\nabla_l V_{mn}=-\Ein^{kl}\Ein^{im}\Ein^{jn}\nabla_l (e^{-\frac{1}{2}u}O_{mn})\\
=&-\frac{1}{\sqrt{\det\Ein}}\Ein^{kl}\Ein^{im}\Ein^{jn}\nabla_l O_{mn}+\frac{1}{2}\mathrm{T}^k\Ein^{ij}.
\end{align}
Thus we obtain
\begin{align*}
\mathrm{T}^{kij}-\mathrm{T}^{ikj}=&\frac{\Ein^{kl}\Ein^{im}\Ein^{jn}}{\sqrt{\det\Ein}}\left(\nabla_m O_{ln}-\nabla_l O_{mn}\right)+\frac{1}{2}(\mathrm{T}^k\Ein^{ij}-\mathrm{T}^i\Ein^{kj}).
\end{align*}
Hence we have the identities
\begin{align}
\mathrm{D}^{kij}&=\frac{\Ein^{kl}\Ein^{im}\Ein^{jn}}{\sqrt{\det\Ein}}\left(\nabla_m O_{ln}-\nabla_l O_{mn}\right)\\
|\mathrm{D}^{ijk}|_V^2\det\Ein&=|\nabla_iO_{jk}-\nabla_jO_{ik}|_{\Ein}^2.
\end{align}
\end{proof}
...
\end{thm}
\begin{proof}The claim follows from Theorem \ref{thm:hyperboliicity} below. Here we give another proof without using a monotone quantity.
...
\]
is non-decreasing. Moreover, \(\frac{d}{dt} I(M_t)= 0\) if and only if the Riemannian universal cover \((M, g = \pi^{\ast} k)\) embeds isometrically into Minkowski space \(\R^{3,1}\) as a locally convex, co-compact, spacelike hypersurface.
\end{thm}
\begin{proof}
By \cite[Proposition 9]{MR2055396}, we have
\begin{align}
(\partial_t-\Box)(\det\Ein)^\eta=(\frac{\eta}{2}|\mathrm{D}^{ijk}|_V^2+\frac{\eta}{2}(1-2\eta)|\mathrm{T}^i|^2_V-2\eta\mathrm{H})(\det\Ein)^\eta,
\end{align}
where $\mathrm{H}=\operatorname{Tr}_g\sigma.$ Therefore, we have the identity
\begin{align}
\frac{d}{dt}\int(\det\Ein)^\eta d\mu=\int (\frac{\eta}{2}|\mathrm{D}^{ijk}|_V^2+\frac{\eta}{2}(1-2\eta)|\mathrm{T}^i|^2_V+(1-2\eta)\mathrm{H})(\det\Ein)^\eta d\mu.
\end{align}
In particular, for $\eta=\frac{1}{2}$ we obtain
\begin{align}
\frac{d}{dt}I(M_t)=\frac{1}{4}\int |\mathrm{D}^{ijk}|_V^2\sqrt{\det\Ein}d\mu.
\end{align}
